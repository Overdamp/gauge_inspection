...
class TritonPythonModel:
    def initialize(self, args):
        ...
        self.logger.log_info("Master Router Initialized (Optimized)")
    ...
    async def _process_analog_gauge(self, img):
        ...
        if seg_res.has_error(): return {"error": "Segmentation failed"}, img
        ...
        sx, sy = orig_w / SEG_SIZE, orig_h / SEG_SIZE
        for seg in segmentations:
            if "mask" in seg: seg["mask"] = [[p[0]*sx, p[1]*sy] for p in seg["mask"]]
            if "bbox" in seg: 
    ...
    async def _run_ocr_pipeline(self, frame, segmentations):
        ...
        final = [s for s in segmentations if s["class"] in target_classes]
        if not final: return []
        ...
        for seg in final:
            ...
            x1, y1, x2, y2 = max(0, bx1-10), max(0, by1-10), min(img_w, bx2+10), min(img_h, by2+10)
            crop_img = frame[y1:y2, x1:x2].copy()
            if crop_img.size == 0: continue
            
            # Optimized Preprocessing
            crop_img = cv2.resize(crop_img, (128, 32), interpolation=cv2.INTER_LINEAR)
    ...
    def _get_gauge_reading(self, img, segmentations, ellipse_result, ocr_results):
        if not ellipse_result: return {"error": "No ellipse found", "debug_img": img}
        center = ellipse_result[0]["center"]
        needle_mask = []
        needle_cands = [s for s in segmentations if s["class"] == "needle"]
        if needle_cands:
            pts = np.array(needle_cands[0]["mask"])
            needle_mask = needle_cands[0]["mask"]
        
        unit = ""
        for o in ocr_results:
            if o["class"] == "unit" and o["confidence"] > 0.5: unit = o["text"]

        result = self.calculator.process_gauge(center, needle_mask, ocr_results, ellipse_result[0])
        debug_img = self._draw_debug(img, center, result, unit)
        if result:
            result.update({"unit": unit, "debug_img": debug_img})
            return result
        return {"error": "Gauge calculation failed", "debug_img": debug_img}

    def _draw_debug(self, img, center, result, unit):
        vis = cv2.resize(img.copy(), (640, 640))
        oh, ow = img.shape[:2]
        sx, sy = 640/ow, 640/oh
        ...
        if result and "value" in result:
            ...
            cv2.putText(vis, f"Read: {result['value']:.2f} {unit}", (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0,255,0), 2)
        return vis
