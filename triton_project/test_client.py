...
def test_triton():
    ...
    # Resize to 640x640 and send as UINT8 tensor (avoids Triton Python Backend BYTES SHM bug)
    ...
    # Prepare inputs - only IMAGE as UINT8 tensor
    inputs = [
        httpclient.InferInput("IMAGE", img_batch.shape, "UINT8"),
    ]
    inputs[0].set_data_from_numpy(img_batch)

    # Send TASK_TYPE as request parameter (avoids multi-input BYTES bug)
    ...
    print(f"กำลังส่งภาพ {os.path.basename(test_image_path)} ไปให้ Triton ประมวลผล...")
    ...
    try:
        ...
        print(f"✅ ฝั่ง Server ตอบกลับสำเร็จ!")
        ...
        output_json_path = os.path.join(output_dir, f"result_{os.path.basename(test_image_path)}.json")
        ...
        if isinstance(img_bytes_out, (bytes, bytearray)) and len(img_bytes_out) > 0:
            ...
            if vis_img is not None:
                output_image_path = os.path.join(output_dir, f"result_{os.path.basename(test_image_path)}")
