...
### 5. มาตรฐานการวางสถาปัตยกรรมตัวแชร์ทรัพยากร (Shared Backbone Architecture Sweet-Spot)

เพื่อหลีกเลี่ยงความซับซ้อนในการจัดการโฟลเดอร์แบบ Ensemble (ที่ต้องสร้าง 3-4 โฟลเดอร์ย่อยต่อหนึ่งโมเดลย่อย) ระบบของเราตกลงใช้แนวทาง **"ตู้เดี่ยวผสม (Hybrid Python Wrapper)"** เป็นมาตรฐานการผลิต:

*   **ฝั่งโมเดลย่อย (`shared_*`):** จัดเก็บเฉพาะไฟล์เวทโมเดลดิบในอาร์ติแฟกต์เวอร์ชัน เช่น `.onnx` หรือ TensorRT `.plan/.engine` ในห้องรหัสเวอร์ชัน `1/` หรือ `2/` ปราศจากไฟล์สคริปต์ Python ในไดเรกทอรี เพื่อให้ Triton เรียกใช้ C++ Backends ที่เร็วและประหยัด VRAM สูงสุด
*   **ฝั่งตรรกะและการนวดข้อมูล (Pre/Post-processing):** ถูกรวมศูนย์ (Centralized) เขียนอยู่ภายในโค้ดเราเตอร์ตรรกะหลักของภารกิจ (`task_*_router/1/model.py`) เช่น การย่อขนาดภาพด้วย OpenCV, สเกลสีเป็นทศนิยม และการทำ Non-Maximum Suppression (NMS)
*   **ผลลัพธ์:** ได้ระบบประมวลผลคู่ขนานยืดหยุ่นสูงที่พัฒนาง่าย ไม่ยุ่งยาก และคงความเร็วสูงสุดในระดับ Enterprise Production!
